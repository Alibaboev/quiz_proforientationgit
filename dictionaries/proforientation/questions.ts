// /dictionaries/quiz/questions.ts

// Мы определяем типы, чтобы TypeScript "понимал" структуру наших вопросов.
// Это помогает избежать ошибок в будущем.
type QuestionType = "multiple-choice" | "open-ended";

interface Question {
  type: QuestionType;
  question: string;
  options?: string[];
  tags?: string[][];
}

interface QuestionSet {
  [key: string]: Question[];
}

interface QuestionBank {
  student: {
    grade_9: QuestionSet;
    grade_11: QuestionSet;
    bachelor: QuestionSet;
    undecided: QuestionSet;
  };
  parent: {
    all: QuestionSet;
  };
}

// Экспортируем все вопросы как константу 'questions'
export const questions = {
  "student": {
    "grade_9": [
      {
        "type": "multiple-choice",
        "question": "Що тобі найцікавіше вивчати?",
        "options": [
          "Як влаштований світ і природа (біологія, хімія)",
          "Як працюють технології та комп'ютери (інформатика, фізика)",
          "Як взаємодіють люди та суспільство (історія, мови, право)",
          "Як працюють гроші та бізнес (математика, економіка)"
        ],
        "tags": [
          ["NAT", "MED"],
          ["TECH"],
          ["HUM"],
          ["ECO", "TECH"]
        ]
      },
      {
        "type": "multiple-choice",
        "question": "Який тип завдань тобі до душі?",
        "options": [
          "Допомагати людям, вирішувати їхні проблеми",
          "Створювати щось нове: програми, механізми, дизайн",
          "Аналізувати інформацію, писати тексти, вивчати мови",
          "Організовувати процеси, керувати командою, рахувати"
        ],
        "tags": [
          ["MED", "HUM"],
          ["TECH"],
          ["HUM"],
          ["ECO"]
        ]
      },
      {
        "type": "open-ended",
        "question": "Уяви свою роботу через 10 років. Опиши одним реченням, чим ти займаєшся?"
      }
    ],
    "grade_11": [
      {
        "type": "multiple-choice",
        "question": "Який напрям підготовки до іспитів (ЗНО/НМТ) тобі найближчий?",
        "options": [
          "Біологія та хімія",
          "Фізика та математика",
          "Історія та іноземні мови",
          "Математика та географія/англійська"
        ],
        "tags": [
          ["MED", "NAT"],
          ["TECH"],
          ["HUM"],
          ["ECO"]
        ]
      },
      {
        "type": "multiple-choice",
        "question": "Що для тебе найважливіше в майбутньому університеті?",
        "options": [
          "Можливість отримати престижний диплом",
          "Максимум практики та стажувань під час навчання",
          "Сильна наукова база та можливість займатися дослідженнями",
          "Доступна вартість навчання та життя в місті"
        ],
        "tags": [
          ["HUM"],
          ["TECH", "ECO"],
          ["MED", "NAT"],
          []
        ]
      },
      {
        "type": "open-ended",
        "question": "Яка твоя найбільша мета при вступі до університету за кордоном?"
      }
    ],
    "bachelor": [
      {
        "type": "multiple-choice",
        "question": "Що є головною метою вашого вступу до магістратури?",
        "options": [
          "Поглибити знання у своїй спеціальності для кар'єрного зростання",
          "Змінити спеціальність на більш перспективну",
          "Отримати європейський диплом для роботи в ЄС",
          "Зайнятися науковою діяльністю, вступити на PhD"
        ],
        "tags": [
          [],
          ["TECH", "ECO"],
          [],
          ["MED", "NAT"]
        ]
      },
      {
        "type": "multiple-choice",
        "question": "Який формат навчання вам більше підходить?",
        "options": [
          "Практичний, з фокусом на проєктах та стажуваннях",
          "Академічний, з поглибленою теорією та дослідженнями",
          "Комбінований, збалансований формат",
          "Вечірній або заочний, щоб поєднувати з роботою"
        ],
        "tags": [
          ["TECH", "ECO"],
          ["MED", "NAT"],
          [],
          []
        ]
      },
      {
        "type": "open-ended",
        "question": "Опишіть коротко ваш попередній досвід (освіта, робота) та очікування від магістерської програми."
      }
    ],
    "undecided": [
      {
        "type": "multiple-choice",
        "question": "Який із цих видів діяльності приваблює тебе найбільше?",
        "options": [
          "Допомагати та лікувати",
          "Винаходити та будувати",
          "Спілкуватися та домовлятися",
          "Аналізувати та рахувати"
        ],
        "tags": [
          ["MED", "NAT"],
          ["TECH"],
          ["HUM"],
          ["ECO"]
        ]
      },
      {
        "type": "multiple-choice",
        "question": "Що для тебе важливіше в роботі?",
        "options": [
          "Стабільність та соціальна значущість",
          "Високий дохід та кар'єрне зростання",
          "Творчість та самовираження",
          "Можливість постійно вчитися новому"
        ],
        "tags": [
          ["MED", "HUM"],
          ["ECO"],
          ["HUM"],
          ["TECH", "NAT"]
        ]
      },
      {
        "type": "open-ended",
        "question": "Якби в тебе була будь-яка можливість, яку одну проблему у світі ти б хотів(ла) вирішити?"
      }
    ]
  },
  "parent": {
    "all": [
      {
        "type": "multiple-choice",
        "question": "Що для вас є головним пріоритетом при виборі освіти для дитини?",
        "options": [
          "Гарантоване працевлаштування після випуску",
          "Престиж університету та якість освіти",
          "Безпека та комфорт проживання в країні",
          "Мінімальні фінансові витрати на навчання та життя"
        ],
        "tags": [
          ["TECH", "ECO"],
          ["HUM", "MED"],
          [],
          []
        ]
      },
      {
        "type": "multiple-choice",
        "question": "Який із цих талантів найяскравіше виражений у вашої дитини?",
        "options": [
          "Емпатія та бажання допомагати іншим",
          "Логічне мислення та любов до техніки",
          "Комунікабельність та гуманітарний склад розуму",
          "Аналітичні здібності та інтерес до бізнесу"
        ],
        "tags": [
          ["MED", "HUM"],
          ["TECH"],
          ["HUM"],
          ["ECO"]
        ]
      },
      {
        "type": "open-ended",
        "question": "Що є вашим найбільшим занепокоєнням щодо вступу дитини за кордон?"
      }
    ]
  }
};
